...
function detailEditBrowser(): string
{
...
function detailEditFileUrl(string $path): string
{
    $path = str_replace('\\', '/', realpath($path) ?: $path);
    return 'file:///' . ltrim(implode('/', array_map('rawurlencode', explode('/', $path))), '/');
}

$profile = sys_get_temp_dir() . '/veciahorra-store-detail-edit-' . getmypid();
$process = proc_open([
    detailEditBrowser(), '--headless=new', '--disable-gpu', '--no-first-run', '--no-default-browser-check',
    '--allow-file-access-from-files', '--user-data-dir=' . $profile, '--virtual-time-budget=5000', '--dump-dom',
    detailEditFileUrl(__DIR__ . '/store-admin-operational-detail-edit-test.html'),
...
echo "PASS store-admin-operational-detail-edit-browser-test {$payload}\n";
