...
usleep(250000);
try {
    $profile = sys_get_temp_dir() . '/orders-detail-navigation-' . getmypid();
    $process = proc_open(
        [
            $browser,
            '--headless=new',
            ...
            '--dump-dom',
            "http://127.0.0.1:{$port}/tests/manual/order-admin-detail-navigation-test.html",
        ],
        [1 => ['pipe', 'w'], 2 => ['pipe', 'w']],
        $io,
        $root
    ...
    if ($exit !== 0 || ! str_contains($html, 'data-status="pass"')) {
        ...
        throw new RuntimeException('Navigation browser fallo: ' . ($result[1] ?? $error));
    }
    preg_match('/<pre id="results"[^>]*>(.*?)<\/pre>/s', $html, $result);
    echo ($result[1] ?? 'PASS order-admin-detail-navigation-browser-test') . PHP_EOL;
} finally {
