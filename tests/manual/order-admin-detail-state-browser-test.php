...
usleep(250000);
try {
    $profile = sys_get_temp_dir() . '/orders-detail-state-' . getmypid();
    $url = "http://127.0.0.1:{$port}/tests/manual/order-admin-detail-state-test.html";
    $process = proc_open(
        [
            $browser,
            '--headless=new',
    ...
    if ($exit !== 0 || ! str_contains($html, 'data-status="pass"')) {
        ...
        throw new RuntimeException('State browser fallo: ' . ($result[1] ?? $error));
    }
    preg_match('/<pre id="results"[^>]*>(.*?)<\/pre>/s', $html, $result);
    echo ($result[1] ?? 'PASS order-admin-detail-state-browser-test') . PHP_EOL;
} finally {
