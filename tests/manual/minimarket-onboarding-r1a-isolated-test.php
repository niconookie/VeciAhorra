...
foreach([['id','8'],['application_id','51'],['purpose','other'],['generation','3'],['candidate_user_id','2'],['attached_user_id','2'],['email_binding_hash',str_repeat('x',32)],['token_hash',str_repeat('y',32)],['expires_at','2026-08-03 00:00:00'],['consumed_at',null],['failed_attempts','1'],['resend_count','2'],['last_sent_at','2026-08-01 00:00:02'],['delivery_state','failed'],['delivery_attempt_count','2'],['last_error_code','delivery_failed'],['created_at','2026-07-31 23:59:59'],['updated_at','2026-08-01 00:00:03']] as [$field,$value]){$wpdb=new R1aOnboardingWpdb();$wpdb->application['status']='account_created';$wpdb->application['user_id']='1';$wpdb->application['updated_at']='2026-08-01 00:00:04';$wpdb->verification[$field]=$value;try{$reconcileRecovery->invoke($onboarding,50,$expectedApplication,$expectedVerification);throw new LogicException('Verification distinta aceptada.');}catch(RuntimeException $exception){isolatedAssert($exception->getMessage()==='onboarding_recovery_conflict'||$exception->getMessage()==='onboarding_recovery_outcome_uncertain','Recovery verification hostil mal clasificada: '.$field);}}
