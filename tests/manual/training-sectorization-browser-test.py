...
try:
 ...
 d.set_window_size(1440,1000);open_product(d,w,"Coca-Cola Original 1,5 L");offers=w.until(lambda x:x.find_element(By.CSS_SELECTOR,"[data-va-offer-list]")).text;assert "Oferta 1" in offers and "Minimarket" not in offers;change(d,w,"2","Coca-Cola Original 1,5 L","");offers=w.until(lambda x:x.find_element(By.CSS_SELECTOR,"[data-va-offer-list]")).text;assert "Oferta 1" in offers and "Minimarket" not in offers;result["product_detail"]="PASS"
 ...
 cart_url=d.current_url;d.get(exclusive_product_url);selector_ready(d,w);w.until(lambda x:not x.find_element(By.CSS_SELECTOR,"[data-va-product-loading]").is_displayed());assert not d.find_elements(By.CSS_SELECTOR,".va-offer-card[data-inventory-id]") and not d.find_element(By.CSS_SELECTOR,"[data-va-add-to-cart]").is_enabled();result["incompatible_addition"]="PASS";d.get(cart_url);w.until(lambda x:x.find_element(By.CSS_SELECTOR,"[data-cart-item-id]"));checkout=d.find_element(By.CSS_SELECTOR,"[data-va-cart-checkout]");click_visible(d,w,checkout);w.until(lambda x:x.find_element(By.CSS_SELECTOR,"[data-va-checkout-content]").is_displayed());submit=d.find_element(By.CSS_SELECTOR,"[data-va-checkout-submit]");phone=d.find_element(By.NAME,"phone");email=d.find_element(By.NAME,"email");phone.send_keys("+56955550000");email.send_keys("carolina.soto.demo@veciahorra.test");click_visible(d,w,submit);errors=w.until(lambda x:x.find_element(By.CSS_SELECTOR,"[data-va-checkout-validation-error-list]"));w.until(lambda x:errors.text.strip()!="");assert "sector" in errors.text.lower();result["checkout_blocked"]="PASS"
