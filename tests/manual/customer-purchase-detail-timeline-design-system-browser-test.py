"""Deterministic Phase 13 browser certification for the purchase timeline."""
...
ROOT_SELECTOR = "[data-va-customer-panel-detail-timeline]"
...
def assert_timeline(driver, expected=1, count=None, empty=False):
    roots=driver.find_elements(By.CSS_SELECTOR,ROOT_SELECTOR);check(len(roots)==expected,f"Cardinalidad Timeline {len(roots)} != {expected}")
    if not expected:return None
    root=roots[0];check(root.tag_name.lower()=="section","Raiz no section.")
    check(root.get_attribute("class").split()==["veciahorra-frontend","va-design-system","va-customer-panel__detail-section","va-customer-panel__timeline"],"Clase literal incorrecta.")
    check(len(root.find_elements(By.CSS_SELECTOR,":scope > h3.va-customer-panel__visual-heading"))==1 and "Timeline" in root.text,"Heading ausente.")
    check(not root.find_elements(By.CSS_SELECTOR,"button,a,form"),"Accion Timeline.")
    lists=root.find_elements(By.CSS_SELECTOR,":scope > ol.va-customer-panel__timeline-list")
    if empty: check(not lists and "No hay eventos para mostrar." in root.text,"Vacio alterado.")
    else:
        check(len(lists)==1,"Lista semantica ausente."); items=lists[0].find_elements(By.CSS_SELECTOR,":scope > li.va-customer-panel__timeline-entry")
        if count is not None: check(len(items)==count,"Cantidad eventos incorrecta.")
        check(all(len(i.find_elements(By.CSS_SELECTOR,":scope > time.va-customer-panel__timeline-time"))==1 for i in items),"time ausente.")
...
try:
    ...
    enter(driver,wait,"loading");wait.until(lambda d:d.execute_script("return window.__vaPhase13Pending.length===1"));check(not driver.find_elements(By.CSS_SELECTOR,ROOT_SELECTOR),"Loading invadido.");driver.execute_script("window.__vaPhase13Pending.shift()()");wait.until(lambda d:len(d.find_elements(By.CSS_SELECTOR,ROOT_SELECTOR))==1);assert_timeline(driver,count=5);result["states"]["loading"]="PASS"
    ...
    enter(driver,wait,"empty");wait.until(lambda d:len(d.find_elements(By.CSS_SELECTOR,ROOT_SELECTOR))==1);assert_timeline(driver,empty=True);result["states"]["empty"]="PASS"
    for code in ("checkout_created","payment_confirmed","payment_reconciled","orders_materialized","delivery_created"):
        enter(driver,wait,code);wait.until(lambda d:len(d.find_elements(By.CSS_SELECTOR,ROOT_SELECTOR))==1);root=assert_timeline(driver,count=1);check(code.split("_")[0].lower() in root.text.lower() or root.text,"Evento ausente.");result["events"][code]="PASS"
    for value,count in (("all",5),("duplicates",3),("ordered",3),("message",1),("long",1),("unknown",1),("invalid_date",1)):
        enter(driver,wait,value);wait.until(lambda d:len(d.find_elements(By.CSS_SELECTOR,ROOT_SELECTOR))==1);root=assert_timeline(driver,count=count)
        if value=="unknown":check("--neutral" in root.find_element(By.CSS_SELECTOR,"li").get_attribute("class"),"Unknown no neutral.")
        if value=="invalid_date":check("Fecha no disponible" in root.text and root.find_element(By.CSS_SELECTOR,"time").get_attribute("datetime")=="invalid-date","Fallback fecha.")
        result["states"][value]="PASS"
    enter(driver,wait,"invalid_type");wait.until(lambda d:"No pudimos cargar" in d.find_element(By.CSS_SELECTOR,"[data-va-customer-panel-content]").text);assert_timeline(driver,0);result["states"]["invalid_type"]="PASS"
    for value,fragment in (("not_found","La compra no est"),("http_error","No pudimos cargar"),("network_error","No pudimos cargar")):
        enter(driver,wait,value);wait.until(lambda d:fragment in d.find_element(By.CSS_SELECTOR,"[data-va-customer-panel-content]").text);assert_timeline(driver,0);result["states"][value]="PASS"
    enter(driver,wait,"stale");wait.until(lambda d:d.execute_script("return window.__vaPhase13Pending.length===1"));driver.back();wait.until(lambda d:len(d.find_elements(By.CSS_SELECTOR,".va-customer-panel__purchase-link"))==1);state(driver,"all");driver.find_element(By.CSS_SELECTOR,".va-customer-panel__purchase-link").click();wait.until(lambda d:len(d.find_elements(By.CSS_SELECTOR,ROOT_SELECTOR))==1);driver.execute_script("window.__vaPhase13Pending.shift()()");assert_timeline(driver,count=5);result["states"]["stale"]="PASS"
    ...
    driver.get(BASE+"?compra="+PUBLIC_ID);wait.until(lambda d:len(d.find_elements(By.CSS_SELECTOR,ROOT_SELECTOR))==1);assert_timeline(driver,count=5);result["direct"]="PASS"
    for width,height in ((1440,1000),(375,812),(320,800)):
        driver.set_window_size(width,height);state(driver,"long");driver.get(BASE+"?compra="+PUBLIC_ID);wait.until(lambda d:len(d.find_elements(By.CSS_SELECTOR,ROOT_SELECTOR))==1);root=assert_timeline(driver,count=1);check(root.is_displayed() and driver.execute_script("return document.documentElement.scrollWidth<=document.documentElement.clientWidth+1"),"Overflow.");back=driver.find_element(By.CSS_SELECTOR,".va-customer-panel__back-link");check(back.rect["width"]>=44 and back.rect["height"]>=44,"Control menor 44.");check(driver.execute_script("return document.activeElement.classList.contains('va-customer-panel__detail-title')"),"Foco inicial.");result["viewports"][f"{width}x{height}"]="PASS"
