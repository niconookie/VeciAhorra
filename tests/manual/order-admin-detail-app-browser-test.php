...
try {
    foreach ([1440, 1024, 768, 375] as $width) {
        $profile = sys_get_temp_dir() . "/orders-detail-app-{$width}-" . getmypid();
        $process = proc_open(
            [
                $browser,
                '--headless=new',
                ...
                '--dump-dom',
                "http://127.0.0.1:{$port}/tests/manual/order-admin-detail-app-test.html",
            ],
            [1 => ['pipe', 'w'], 2 => ['pipe', 'w']],
            $io,
            $root
        ...
        if ($exit !== 0 || ! str_contains($html, 'data-status="pass"')) {
            ...
            throw new RuntimeException("Application browser {$width} fallo: " . ($result[1] ?? $error));
        }
        preg_match('/<pre id="results"[^>]*>(.*?)<\/pre>/s', $html, $result);
        echo "width={$width} " . ($result[1] ?? 'PASS') . PHP_EOL;
    }
} finally {
