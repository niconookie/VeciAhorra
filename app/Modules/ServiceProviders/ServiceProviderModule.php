...
final class ServiceProviderModule
{
    ...
    private function providerForm(bool $wizard, ?string $selectedPlan = null): string
    {
        $steps = $wizard ? ' data-va-provider-wizard' : '';
        $checked = static fn (string $plan): string => $selectedPlan === $plan ? ' checked' : '';
        return '<form id="va-provider-form" class="va-sp-provider-form" data-va-provider-form' . $steps . '>'
            ...
            . '<label><input type="radio" name="plan" value="local" required' . $checked('local') . '><span></span><strong>Plan Local</strong><b>$1.000 <small>/ mes</small></b><p>Perfil verificado y aparición en búsquedas de tu comuna.</p><em>Ideal para comenzar</em></label>'
            . '<label><input type="radio" name="plan" value="featured" required' . $checked('featured') . '><span></span><strong>Plan Destacado</strong><b>$2.000 <small>/ mes</small></b><p>Posición preferente, sello destacado y acceso a campañas.</p><em>Mayor exposición</em></label>'
            . '<label><input type="radio" name="plan" value="communal" required' . $checked('communal') . '><span></span><strong>Plan Comunal</strong><b>$3.000 <small>/ mes</small></b><p>Incluye Plan Destacado y visibilidad informativa en toda la comuna.</p><em>Alcance comunal</em></label></div></section>'
    ...
    public function landing(): string
    {
        ...
        $requestedPanelUrl = $requestedPlan === null
            ? $panelUrl
            : add_query_arg('plan', $requestedPlan, $panelUrl);
        $isProvider = is_user_logged_in() && current_user_can(ServiceProviderRole::CAPABILITY);
        $registrationUrl = is_user_logged_in()
            ? $requestedPanelUrl
            : $this->registrationUrl($requestedPlan);
        $planUrls = [];
        foreach (array_keys(ServicePlanCatalog::PLANS) as $plan) {
            $planUrls[$plan] = is_user_logged_in()
                ? add_query_arg('plan', $plan, $panelUrl)
                : $this->registrationUrl($plan);
        }
        $primaryAction = $isProvider
            ? '<a class="va-sp-button" href="' . esc_url($requestedPanelUrl) . '">Ir a mi panel</a>'
            : (is_user_logged_in()
                ? '<button class="va-sp-button" type="button" data-va-provider-enroll data-va-provider-panel-url="' . esc_url($requestedPanelUrl) . '">Activar mi perfil profesional</button><span role="status" data-va-provider-enroll-status></span>'
        ...
        return '<main class="va-sp va-sp-landing">'
            ...
            . '<section class="va-sp-section va-sp-plans" id="planes"><div class="va-sp-section-heading left"><p class="va-sp-eyebrow">Planes informativos</p><h2>Elige cuánta visibilidad necesita tu servicio</h2><p>Estos valores son informativos. El registro no realiza cobros ni suscripciones.</p></div><div class="va-sp-plan-grid"><article><h3>Plan Local</h3><strong><small>$</small>1.000 <span>/ mes</span></strong><p>Para comenzar a mostrar tus servicios en tu comuna.</p><ul><li>✓ Perfil público verificado</li><li>✓ Aparición en búsquedas locales</li><li>✓ Datos de contacto y horarios</li><li>✓ Panel de consultas</li></ul><a href="' . esc_url($planUrls['local']) . '" class="va-sp-button secondary" data-va-choose-plan="local">Elegir Plan Local</a></article><article class="featured"><span class="va-sp-popular">Mayor exposición</span><h3>Plan Destacado</h3><strong><small>$</small>2.000 <span>/ mes</span></strong><p>Para aparecer antes y participar en acciones comerciales.</p><ul><li>✓ Todo lo incluido en Plan Local</li><li>✓ Posición preferente en resultados</li><li>✓ Sello “Destacado” en el perfil</li><li>✓ Acceso prioritario a campañas</li></ul><a href="' . esc_url($planUrls['featured']) . '" class="va-sp-button" data-va-choose-plan="featured">Elegir Plan Destacado</a></article><article class="communal"><h3>Plan Comunal</h3><strong><small>$</small>3.000 <span>/ mes</span></strong><p>Para ampliar la presencia del servicio en su territorio.</p><ul><li>✓ Todo lo incluido en Plan Destacado</li><li>✓ Posición preferente en resultados</li><li>✓ Sello “Destacado” en el perfil</li><li>✓ Acceso prioritario a campañas</li><li>✓ Visibilidad en toda la comuna</li></ul><a href="' . esc_url($planUrls['communal']) . '" class="va-sp-button" data-va-choose-plan="communal">Elegir Plan Comunal</a></article></div></section>'
    ...
    private function requestedPlan(): ?string
    {
        return ServicePlanCatalog::canonical(
            isset($_GET['plan']) ? wp_unslash($_GET['plan']) : null
        );
    }

    private function registrationUrl(?string $plan): string
    {
        $returnUrl = $this->pageUrl(self::REGISTRATION, '/prestadores/');
        if ($plan !== null) {
            $returnUrl = add_query_arg('plan', $plan, $returnUrl);
        }
        return $this->customerAccess->registrationUrl($returnUrl);
    }
