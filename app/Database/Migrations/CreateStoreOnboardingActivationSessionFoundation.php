...
final class CreateStoreOnboardingActivationSessionFoundation
{
    ...
    private function assertTable(array $expected,array $indexes):void{global $wpdb;$table=$this->table();$wpdb->last_error='';$meta=$wpdb->get_row($wpdb->prepare('SELECT ENGINE,TABLE_COLLATION FROM information_schema.TABLES WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME=%s',$table),ARRAY_A);if(!is_array($meta)||$wpdb->last_error!==''||strtoupper((string)$meta['ENGINE'])!=='INNODB')throw new RuntimeException('r1dca_session_schema_invalid');$this->assertCharset((string)$meta['TABLE_COLLATION']);$rows=$wpdb->get_results("SHOW COLUMNS FROM {$table}",ARRAY_A);if(!is_array($rows)||$wpdb->last_error!=='')throw new RuntimeException('r1dca_session_schema_inspection_failed');$actual=[];foreach($rows as $r)$actual[(string)$r['Field']]=$r;if(array_keys($actual)!==array_keys($expected))throw new RuntimeException('r1dca_session_schema_invalid');foreach($expected as $name=>[$type,$null,$default,$extra]){$r=$actual[$name];if(!$this->type((string)$r['Type'],$type)||$r['Null']!==$null||($r['Default']??null)!==$default||strtolower((string)$r['Extra'])!==$extra)throw new RuntimeException('r1dca_session_schema_invalid');}$all=$wpdb->get_results("SHOW INDEX FROM {$table}",ARRAY_A);if(!is_array($all)||$wpdb->last_error!=='')throw new RuntimeException('r1dca_session_schema_inspection_failed');$group=[];foreach($all as $r)$group[(string)$r['Key_name']][]=$r;if(array_diff_key($group,$indexes)!==[]||array_diff_key($indexes,$group)!==[])throw new RuntimeException('r1dca_session_schema_invalid');foreach($indexes as $name=>[$cols,$unique]){$rs=$group[$name];usort($rs,fn($a,$b)=>(int)$a['Seq_in_index']<=>(int)$b['Seq_in_index']);if(array_map(fn($r)=>(string)$r['Column_name'],$rs)!==$cols||array_map(fn($r)=>(int)$r['Seq_in_index'],$rs)!==range(1,count($cols)))throw new RuntimeException('r1dca_session_schema_invalid');foreach($rs as $r)if((int)$r['Non_unique']!==($unique?0:1)||($r['Sub_part']??null)!==null)throw new RuntimeException('r1dca_session_schema_invalid');}}
    private function assertCharset(string $collation):void{global $wpdb;$contract=(string)$wpdb->get_charset_collate();if(!preg_match('/CHARACTER SET\s+([^\s]+)/i',$contract,$c)||!preg_match('/COLLATE\s+([^\s]+)/i',$contract,$x))throw new RuntimeException('r1dca_session_schema_invalid');$charset=$wpdb->get_var($wpdb->prepare('SELECT CHARACTER_SET_NAME FROM information_schema.COLLATIONS WHERE COLLATION_NAME=%s',$collation));if(strtolower((string)$charset)!==strtolower($c[1])||strtolower($collation)!==strtolower($x[1]))throw new RuntimeException('r1dca_session_schema_invalid');}
