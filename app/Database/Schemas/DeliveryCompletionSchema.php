...
final class DeliveryCompletionSchema implements TableInterface
{
    public function name(): string { return 'delivery_completions'; }
    ...
    public function define(TableBuilder $table): void
    {
        $table->id()
            ...
            ->datetime('created_at')
            ->datetime('updated_at')
            ->unique('business_completion_id', 'delivery_completions_business_unique')
            ->unique('idempotency_key', 'delivery_completions_key_unique')
            ->index(['completion_status', 'lease_expires_at'], 'delivery_completions_claim_index');
    }
}
